...
- `mail-service` - Kafka-dan reset event oxuyub SMTP ilə email göndərir.
...
.\gradlew.bat :mail-service:bootRun
```
...
## Forgot/Reset password axini

- `POST /auth/forgot-password` cagrisi `auth.password-reset` topic-ine Kafka event yazir.
- `mail-service` bu topic-i dinleyib `resetUrl` ile email gonderir.
- User linke daxil olub `POST /auth/reset-password` ile sifresini yenileyir.

Mail ayarlari ucun:
- `.env.example` faylini nusxeleyib oz deyerlerini yaz.
- Gmail ucun `MAIL_PASSWORD` olaraq App Password istifade et.
