...
public interface ProductService {
    ...
    Page<ProductBySlugResponse> getViewAllProducts(int size, int page, Boolean isSingle);

    void deleteProduct(Long id);

    UpdateProductResponse updateProductResponse(Long id, UpdateProductRequest request);

    Page<ProductByCategoryResponse> getProductByCategory(String categoryKey, int size, int page, Boolean isSingle);

    List<ProductBySlugResponse> filterProducts(
            Long categoryId, String category, BigDecimal minPrice, BigDecimal maxPrice, Boolean isSingle);
}
